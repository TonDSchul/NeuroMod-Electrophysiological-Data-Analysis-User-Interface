function  [Success] = Spike_Module_Start_Phy(Data,executableFolder,resultsFolder)
...
[PhyPython_Conda_Environment_Path] = Spike_Module_Load_Phy_Conda_Python_exe(executableFolder);
...
if isempty(PhyEnvName) 
    msgbox("Name of the anaconda environment Phy is installed in could not be identified! Please select the python.exe within the folder of the environment with Phy.")
    return
end

if ~isfile(CondaBat)
    msgbox("Conda .bat file in the anaconda root folder could not be found! Please select the python.exe within the folder of the environment with Phy. This folder should be within the 'envs' folder of the anaconda root folder.")
    return
end
