function [Data,SaveFilter] = Spike_Module_Load_Kilosort_Data(Data,Autorun,SelectedFolder,ScalingFactor,KSVersion,DeleteMUA,SpikeChannelType,LoadSpikesinTimeRange,ParallelPool)
...
% 9: ParallelPool: handle to parallel preprocessing pool that either
...
for i = 1:length(fileNames)
    ...
    elseif strcmp(fileNames{i},'templates.npy')
        ...
        % get max tempalte channel for each unit
        PeaktoPeak = squeeze(max(Data.Spikes.templates, [], 2) - min(Data.Spikes.templates, [], 2));
        % Max channel per cluster
        [~, Data.Spikes.Cluster_Max_Template_Channel] = max(PeaktoPeak, [], 2);
...
if strcmp(SpikeChannelType,"Channel closest to X and Y of respective spikes")
    ...
    Data = Spike_Module_Get_Spike_Channel_Max_Template(Data,"Kilosort");
end
