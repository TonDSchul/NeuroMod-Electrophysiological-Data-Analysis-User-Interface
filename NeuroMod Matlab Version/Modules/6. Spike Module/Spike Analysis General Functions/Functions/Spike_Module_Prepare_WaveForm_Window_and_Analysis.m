...
MaxNrUnits = length(UniqueClus);
%% Prepare inputs

Units = cell(1,2);

for nUnits = 1:2
    PlotIndice = eval(strcat("U",num2str(nUnits)));
    UnitsInPlot = str2double(strsplit(PlotIndice,','));

    for i = 1:length(UnitsInPlot)
        if sum(UnitsInPlot(i)==UniqueClus)>0
            Units{nUnits}(i)=UnitsInPlot(i);
        else
            msgbox("Unit was not found!");
            warning("Unit was not found!")
        end
    end

    % if nUnits == 1
    %     if length(UniqueClus)>=2
    %         Units{1}(1) = UniqueClus(1);
    %         Units{1}(2) = UniqueClus(2);
    %     end
    % else
    %     if length(UniqueClus)>=4
    %         Units{2}(1) = UniqueClus(3);
    %         Units{2}(2) = UniqueClus(4);
    %     end
    % end
end % Unit loop
