...
if ~strcmp(Clustertoshow,"All") && ~strcmp(Clustertoshow,"Non")
    ...
    %% For RGB Map Indice
    UniqueCluster = unique(ClusterIdentity);
    clusIndice = find(UniqueCluster==Clustertoshow);
    ...
    if sum(IndiciesCurrentCluster)>0
        ...
        Clusterline = line(Figure,spikeTimes,spikePositions,'LineStyle', 'none', 'Marker', 'o','MarkerFaceColor', rgbMatrix(clusIndice,:),'MarkerEdgeColor',rgbMatrix(clusIndice,:),'MarkerSize',PlotAppearance.InternalEventSpikePlot.MainPlotSpikeWidth, 'Tag', 'SpikeRateCluster');
