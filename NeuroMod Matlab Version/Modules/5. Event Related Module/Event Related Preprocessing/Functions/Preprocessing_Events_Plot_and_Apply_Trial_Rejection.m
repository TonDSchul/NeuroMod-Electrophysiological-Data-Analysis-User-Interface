...
% if used outside of GUI where this cannot be controlled
if ThresholdSign(1)==0 && ThresholdSign(2)==0
    ThresholdSign = [1,1];
    disp("Trial Rejection: Positive and negative threshold sign both turned off which is not valid. Auto adjusted to show crossings for positive and negative threshold crossings.")
end
...
for ntrials = AllTrials
    if ~isnan(ntrials) %~isnan(AllTrials(ntrials))
        if ThresholdSign(1)==1 && ThresholdSign(2)==1 % if positive and begative crossings
        ...
        elseif ThresholdSign(1)==1 && ThresholdSign(2)==0 % only postive crossings
            if max(EventRelatedData(ChannelSelection,ntrials,:)) > abs(Threshold)
                ThresCrossingTrials = [ThresCrossingTrials,ntrials];
            end
        elseif ThresholdSign(1)==0 && ThresholdSign(2)==1 % only negative crossings
            if min(EventRelatedData(ChannelSelection,ntrials,:)) < -abs(Threshold)
                ThresCrossingTrials = [ThresCrossingTrials,ntrials];
            end
        elseif ThresholdSign(1)==0 && ThresholdSign(2)==0 % not allowed by GUI
...
if length(PosThreshHandle)>1
    delete(PosThreshHandle(2:end));
    PosThreshHandle = findobj(BottomFigure, 'Tag', 'PosThresh');
end
if length(NegThreshHandle)>1
    delete(NegThreshHandle(2:end));
    NegThreshHandle = findobj(BottomFigure, 'Tag', 'NegThresh');
end
if ThresholdSign(1)==0
    delete(PosThreshHandle);
end
...
% plot threshold
if PlotThreshold == 1 
    %ThreshHandle = [NaN NaN];
    if ThresholdSign(1)==1
        if isempty(PosThreshHandle)
            threshline(1) = line(BottomFigure,[Time(1),Time(end)],[Threshold,Threshold],'Color','g','LineStyle','--','LineWidth',2,'Tag','PosThresh');
        else
            set(PosThreshHandle, 'XData', [Time(1),Time(end)], 'YData',[Threshold,Threshold],'Color','g','LineStyle','--','LineWidth',2,'Tag','PosThresh');
            threshline(1) = PosThreshHandle;
        end
    end
    
    if ThresholdSign(2)==1
        if isempty(NegThreshHandle)
