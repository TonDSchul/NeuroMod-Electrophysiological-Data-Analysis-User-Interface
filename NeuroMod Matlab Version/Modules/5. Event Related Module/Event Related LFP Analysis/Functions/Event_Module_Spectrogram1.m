function [CurrentClim,CurrentPlotData] = Event_Module_Spectrogram1(Data,EventRelatedData,Window,BaselineNormalize,BaselineWindow,TF,CurrentClim,Figure,SampleRate,PlotAppearance,Time,CurrentPlotData,TwoORThreeD,Smoothing)
...
%% Spec and conversion
[power, TFFreqs, TFTime] = cwt(EventRelatedData, SampleRate, 'amor', 'FrequencyLimits', [TF.FreqRange(1) TF.FreqRange(3)],'VoicesPerOctave', Smoothing); % with armor its moorlet wavelets otherwise Morse 
...
ylim(Figure,[min(TFFreqs) max(TFFreqs)])
