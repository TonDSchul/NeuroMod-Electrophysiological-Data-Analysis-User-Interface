    ...
    if ~isempty(PreprocessingSteps)
        ...
        for i = 1:length(PreprocessingSteps)
            if strcmp(PreprocessingSteps(i),"Normalize")
                msgbox("Normalization was already added.");
                % function ends here, next lines only get executed when
    ...
    if ~isempty(PreprocessingSteps)
        ...
        for i = 1:length(PreprocessingSteps)
            ...
            if strcmp(PreprocessingSteps(i),"GrandAverage")
                msgbox("GrandAverage was already added.");
                % function ends here, next lines only get executed when
...
if strcmp(type,"StimArtefactRejection")
    if ~isempty(PreprocessingSteps)
        ...
        for i = 1:length(PreprocessingSteps)
            ...
            if strcmp(PreprocessingSteps(i),"Stimulation Artefact Rejection")
                msgbox("Stim Artefact Rejection was already added.");
                % function ends here, next lines only get executed when
    ...
    % later execution  of the pipeline
        if AlreadyFound == 0
            ...
            Info.ArtefactRejectedTrigger = ArtefactRejectionInfo.SelectedTriggertChannel;
