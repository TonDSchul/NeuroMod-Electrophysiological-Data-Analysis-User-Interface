...
if Activated
    ...
    if strcmp(Window,"MainWindow") || strcmp(Window,"All")
        app.TimeRangeViewBox.Tooltip = "Edit to change time range plotted in main window; Format: Number followed by a 's' i.e. 1s.";
        app.Button.Tooltip = "Click to increase plotted time range by the amount specified in 'Time to Manipulate Main Plot' dropdown menu.";
        app.Button_2.Tooltip = "Click to decrease plotted time range by the amount specified in 'Time to Manipulate Main Plot' dropdown menu.";
        ...
        app.PlayButton.Tooltip = "Click to start play data as a movie. Right click to change number of samples jumped each frame as well as an additional playback speed delay.";
        ...
        app.Slider.Tooltip = "Click and slide to change sclaing of plotted data lines. Right click to change maximum/minimum values. Alternatively press the up or down arrow keys. Bigger spacing leads to a smaller scaling of plotted traces due to bigger ylimits.";
        ...
        app.TextArea.Tooltip = "See all additional information about the recording. This includes info about extracted data, preprocessing, spike and event detection and so on for later referencing.";
    end
    ...
    if strcmp(Window,"ExtractDataWindow") || strcmp(Window,"All")
        if ~isempty(app.ExtractDataWindow) && isvalid(app.ExtractDataWindow)
            ...
            app.ExtractDataWindow.FormatToSaveandReadintoMatlabDropDown.Tooltip = "When using the NEO library to extract recording data, the dataset within python is saved and subsequently loaded into Matlab. The format it is saved in is a custom .dat and .mat file format OR the .mat format NEO officially supplies a function for to save in. In practice this makes no difference except that the custom format can handle larger recordings (when trying to save too much data in a .mat file you will get the error: Python int too large to convert to C long). On the other hand, the NEO created .mat file contains all NEO meta-data, not only the necessary ones to run NeuroMod like in the custom format.";
            app.ExtractDataWindow.Save_Probe_SpikeInterface.Tooltip = "Since the probe design (mapping) is extracted using SpikeInterface, you have to specify whether you want to save it for later use while data is extracted. Save location is the folder data is saved in after data extraction (parent folder of recording folder/{Recording name} SpikeInterface SaveFile). These files can be loaded into Kilosort or SpikeInterface to set probe design.";
            
            %app.ExtractDataWindow.AdditionalAmplificationFactorEditField.Tooltip = "Choose which channel to extract from all available amplifier channel in the recording. Must contain as many channel as active channel specified in the probe design. NOTE: Indexing of amplifier channel/active cahnnel, not active channel numbers themselves. So if first active channel is channel 9, write a 1 here to extract it.";
            ...
            app.ExtractDataWindow.AdditionalAmplificationFactorEditField_2.Tooltip = "Choose which channel to extract from all available amplifier channel in the recording. Must contain as many channel as active channel specified in the probe design. NOTE: Indexing of amplifier channel/active cahnnel, not active channel numbers themselves. So if first active channel is channel 9, write a 1 here to extract it. Format: 'All' to extract all channel, otherwise Matlab expressions like [1,2,3,5]. If you want to use the NEO library and enable the checkbox 'Load already saved .dat file', this field is automatically populated with the value set when extracting the recording. It is requiring the saved NEO_Saved_MetaData.mat file saved in the same folder as the .dat file from NEO data extraction.";
            app.ExtractDataWindow.AdditionalAmplificationFactorEditField_3.Tooltip = "Specify time range to extract from the raw recording. Format: 'from,to' in seconds like '0,Inf' for the whole time range or '0,10' for the first ten seconds. NOTE: For Open Ephys recordings, multiple acquisition starts within one recording node can be combined to a single recording. Time to extract is taken from EVERY recording acquisition period. With two recordings and time to extract = '1,10', 20 seconds are extracted in total, 10 from each. If you want to use the NEO library and enable the checkbox 'Load already saved .dat file', this field is automatically populated with the value set when extracting the recording. Its is requiring the saved NEO_Saved_MetaData.mat file saved in the same folder as the .dat file from NEO data extraction.";
        end
    end
        ...
        if ~isempty(app.ExtractDataWindow) && isvalid(app.ExtractDataWindow)
            if ~isempty(app.ExtractDataWindow.ProbeLayoutWindow)
                if isprop(app.ExtractDataWindow.ProbeLayoutWindow,'ProbeLayoutWindowUIFigure')
                    app.ExtractDataWindow.ProbeLayoutWindow.NrChannelEditField.Tooltip = "Edit number of channel rows of your probe. This number times the number entered in 'Nr Probe Columns' equals the total channel count. Necesary to plot.";
                    app.ExtractDataWindow.ProbeLayoutWindow.ChannelSpacingumEditField.Tooltip = "Edit the spacing (pitch) between channel columns in µm. Necessary to plot.";
                    app.ExtractDataWindow.ProbeLayoutWindow.ChannelRowsDropDown.Tooltip = "Edit the number of channel columns per shank. Second channel column is to the right of the first one. Note: For Neuropixels probe designs ~= Phase 3B, select 2 channel rows and activate the 'Horizontal Offset Between Every Second Column' checkbox!";
                    app.ExtractDataWindow.ProbeLayoutWindow.HorizontalOffsetumEditField.Tooltip = "Edit distance between channel columns; in µm. Must be bigger than 0 µm if probe has more than 1 channel column!";
                    app.ExtractDataWindow.ProbeLayoutWindow.VerticalOffsetumEditField.Tooltip = "Only applicable if two columns are selected. Edit vertical offset of second channel column on the right compared to the first channel column; in µm.";
            
                    app.ExtractDataWindow.ProbeLayoutWindow.VerticalOffsetumEditField_2.Tooltip = "Horizontal distance by which even channel rows are shifted to the right; Should be smaller than the horizontal distance between channel columns; in µm. Note: For Neuropixels probe designs ~= Phase 3B, select 2 channel columns and activate the 'Horizontal Offset Between Every Second Column' checkbox!";
                    app.ExtractDataWindow.ProbeLayoutWindow.CheckBox.Tooltip = "Only applicable if two channel columns selected. When activated, even channel rows are shifted to the right (still counts as two channel columns when two columns are selected).";
    ...
    if strcmp(Window,"ProbeViewMainWindow") || strcmp(Window,"All")
        if ~isempty(app.ProbeViewWindowHandle) && isvalid(app.ProbeViewWindowHandle)
            if isprop(app.ProbeViewWindowHandle,'ProbeViewUIFigure') || isfield(app.ProbeViewWindowHandle,'ProbeViewUIFigure')
                ...
                app.ProbeViewWindowHandle.ShowChannelSpacingCheckBox.Tooltip = "Activate to preserve probe distances between channel in analysis plots (when active channnel are divided into individualy 'islands' delimited by inactive channel inbetween). NOTE: If you plot the Grid View and the probe contains inactive channel islands, the grid is NOT filled according to the true probe design. Leave this activated for acurate spatial relations!";
            end
        end
    end
    %% Live Spike Rate
    ...
    if strcmp(Window,"EventExtraction") || strcmp(Window,"All")
        if ~isempty(app.EventExtractionWindow) && isvalid(app.EventExtractionWindow) 
            ...
            app.EventExtractionWindow.SetFoldermanuallyButton.Tooltip = "When no event data was found in automatically searched folder, select a folder manually. The required folder is a recording folder containing event data. When data was loaded with NEO and a recording folder was selected here, NeuroMod searches for the folder NEO created when extracting the recording ('Name_of_Recording Neo SaveFile' in the parent directory of the selected recording folder). When the folder or event data saved in there was not found, a Python script opens to extract event data from the raw recording files.";
