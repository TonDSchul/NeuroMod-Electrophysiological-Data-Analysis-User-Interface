        ...
        if ~isempty(app.ExtractDataWindow) && isvalid(app.ExtractDataWindow)
            if ~isempty(app.ExtractDataWindow.ProbeLayoutWindow)
                if isprop(app.ExtractDataWindow.ProbeLayoutWindow,'ProbeLayoutWindowUIFigure')
                    ...
                    app.ExtractDataWindow.ProbeLayoutWindow.VerticalOffsetumEditField_2.Tooltip = "Vertical offset between indented channels; in µm. Can be positive or negative, depending on the direction of indentation.";
                    app.ExtractDataWindow.ProbeLayoutWindow.CheckBox.Tooltip = "When activated, even channel rows are indented (still counts as two channel rows when two rows is selected).";
   
                    app.ExtractDataWindow.ProbeLayoutWindow.ActiveChannelField.Tooltip = "Active channel are those that you recorded from in respect to the probe geometry on the right. [1,2,3] means that the first three channel of the Probe design on the right represent the first three channel you recorded from. Number of channel specified here has to be the same as channel found in your recording (BUT can be less than the number of all channel on the probe). Leave empty to mark channel as active. Note 1: When two channel rows are selected and this field is empty, number of active channel is nr of channel*2! Note 2: The top channel used in every analysis is the lowest channel number if not reversed and the highest channel number if reversed";
