        ...
        if ~isempty(app.ExtractDataWindow) && isvalid(app.ExtractDataWindow)
            if ~isempty(app.ExtractDataWindow.ProbeLayoutWindow)
                if isprop(app.ExtractDataWindow.ProbeLayoutWindow,'ProbeLayoutWindowUIFigure')
                    app.ExtractDataWindow.ProbeLayoutWindow.NrChannelEditField.Tooltip = "Edit number of channel columns of your probe. This number times the number entered in 'Nr Probe Rows' equals the total channel count. Necesary to plot.";
                    ...
                    app.ExtractDataWindow.ProbeLayoutWindow.VerticalOffsetumEditField_2.Tooltip = "Horizontal distance by which even channel columns are shifted to the right; Should be smaller than the horizontal distance between channel rows; in µm. Note: For Neuropixels probe designs ~= Phase 3B, select 2 channel rows and activate the 'Horizontal Offset Between Every Second Column' checkbox!";
