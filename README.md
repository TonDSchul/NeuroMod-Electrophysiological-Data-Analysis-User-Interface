...
After a raw recordings was extracted, NeorMod also offers to save data in different formats to load into different Matlab or Python toolboxes that natively would not have worked with your recording format. These formats are .mat files compatible and readable with the NEO python library, .nwb files, costume .mat and .dat files (For future use in NeuroMod or by yourself, not compatible with other toolboxes) as well as SpikeInterface compatible numpy arrays. Being able to read these saved files in NEO and SpikeInterface means, that you can use most of the most popular python toolboxes for spiek sorting and data analysis which use these toolboxes as a data extraction foundation. 

**NOTE:** Although data can be saved in the .nwb and loaded back into Matlab, it cannot be recommended anymore to use NWB in Matlab. The documentation is out of date and the simplest examples with simulated data don't work anymore. Besides this, a documentation focussing to explain how to load an image from the dataset rather than the actual raw data is frustratingly missing the point. Moreover, the data representation for the purpose of the most general ephys experiments in uncessarily complicated, making it almost unusable for inexperienced users. Constant version upgrades and incompatibilities makes it equally impossible to maintain this in a reasonable way.
