...
After a raw recordings was extracted, NeuroMod also offers to save data in different formats to load into different Matlab or Python toolboxes that natively would not have worked with your recording format in the first place. These formats are .mat files compatible and readable with the NEO python library, .nwb files, costume .mat and .dat files (for future use in NeuroMod or by yourself, not compatible with other toolboxes) as well as SpikeInterface compatible numpy arrays. Being able to read these saved files in NEO and SpikeInterface means, that you can use most of the most popular python toolboxes for spike sorting and data analysis which use these toolboxes as a data extraction foundation. 
