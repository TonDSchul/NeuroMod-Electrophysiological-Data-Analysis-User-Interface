    ...
    if strcmp(SpikePlot,"Spikes") && strcmp(SpikeDatatype,"Internal")
        ...
        if ~isempty(SpikeindiciesinRangeButAtStart)
            ...
            for i = 1:numel(SpikePositions) % Channel
                numspikesplotted = numspikesplotted+1;
                if numspikesplotted <= length(SpikeHandles) && ~isempty(SpikeHandles) 
                    set(SpikeHandles(numspikesplotted), 'XData', Time(1:SpikeIndicies(i)+Samplestoshowaroundspike), 'YData', Data(SpikePositions(i),1:SpikeIndicies(i)+Samplestoshowaroundspike),'LineWidth',2.5,'Color','r', 'Tag', 'Spikes');
                else 
                    line(UIAxis, Time(1:SpikeIndicies(i)+Samplestoshowaroundspike), Data(SpikePositions(i),1:SpikeIndicies(i)+Samplestoshowaroundspike),'LineWidth',2.5,'Color','r', 'Tag', 'Spikes');
                end
            end  
        end
        SpikeIndicies = [];
        ...
        if ~isempty(SpikeindiciesinRangeButAtEnd)
            ...
            for i = 1:numel(SpikePositions) % Channel
                numspikesplotted = numspikesplotted+1;
                if numspikesplotted <= length(SpikeHandles) && ~isempty(SpikeHandles) 
                    set(SpikeHandles(numspikesplotted), 'XData', Time(SpikeIndicies(i)-Samplestoshowaroundspike:length(Time)), 'YData', Data(SpikePositions(i),SpikeIndicies(i)-Samplestoshowaroundspike:length(Time)),'LineWidth',2.5,'Color','r', 'Tag', 'Spikes');
                else 
                    line(UIAxis, Time(SpikeIndicies(i)-Samplestoshowaroundspike:length(Time)), Data(SpikePositions(i),SpikeIndicies(i)-Samplestoshowaroundspike:length(Time)),'LineWidth',2.5,'Color','r', 'Tag', 'Spikes');
                end
            end  
        end
        SpikeIndicies = [];
