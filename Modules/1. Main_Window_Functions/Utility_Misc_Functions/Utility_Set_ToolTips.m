        ...
        if ~isempty(app.ExtractDataWindow) && isvalid(app.ExtractDataWindow)
            if ~isempty(app.ExtractDataWindow.ProbeLayoutWindow)
                if isprop(app.ExtractDataWindow.ProbeLayoutWindow,'ProbeLayoutWindowUIFigure')
                    ...
                    app.ExtractDataWindow.ProbeLayoutWindow.ReverseTopandBottomChannelNumberCheckBox.Tooltip = "Enabling this will flip channel 1 from the top to the bottom of the probe. It only changes the channel names, not how data is loaded or displayed.";
                    app.ExtractDataWindow.ProbeLayoutWindow.ReverseTopandBottomChannelNumberCheckBox_2.Tooltip = "Enabling this will flip the extracted datset so that the first extracted channel becomes the last one and the last extracted channel becomes the first one. Note: The top channel used in every analysis is the lowest channel number if not reversed and the highest channel number if reversed!";
