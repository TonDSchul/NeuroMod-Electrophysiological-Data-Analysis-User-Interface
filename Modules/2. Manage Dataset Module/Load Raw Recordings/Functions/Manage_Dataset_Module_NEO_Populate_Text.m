function [ProbeInfoText,RecordingSystemDropDownItems,FileTypeDropDownItems,stringArray,SelectedFolder] = Manage_Dataset_Module_NEO_Populate_Text(ProbeInfo,PathToOpen,JustCheck,SelectedFolder,RecordingType)
...
if JustCheck == 0
    ...
    if strcmp(RecordingType,"Plexon") || strcmp(RecordingType,"Blackrock") || strcmp(RecordingType,"NeuroExplorer")
        disp("Please select a single file with your recording.")
        if isfolder(PathToOpen)
            SelectedFolder = uigetfile(PathToOpen,'Select a file with your recording data','*.*');
        else
            SelectedFolder = uigetfile('Select a file with your recording data','*.*');
        end
