function [DatFilePaths,AmplifierDataIndex,DIChannel,TimeChannel,InfoRhd,selectedFolder] = LoadIntanDatFiles(selectedFolder)
...
% 3. DIChannel: double vector with index of event channel
% contents (index of DatFilePaths)
...
if isempty(AmplifierDataIndex) 
    disp("Error: No .dat file for amplifier channel found.")
elseif isempty(DIChannel)
    disp("Error: No .dat file for event trigger found.")
elseif isempty(TimeChannel)
    disp("Error: No .dat file time found.")
elseif isempty(InfoRhd)
    disp("Error: No info.rhd file found.")
end
