...
if ~strcmp(type,"Extracting")
    Channelnames.Aux = {};
    if ~isempty(aux_input_channels)
        for i = 1:length(aux_input_channels)
            Channelnames.Aux{i} = aux_input_channels(i).custom_channel_name;
        end
    end
    Channelnames.Analog = {};
    if ~isempty(board_adc_channels)
        for i = 1:length(board_adc_channels)
            Channelnames.Analog{i} = board_adc_channels(i).custom_channel_name;
        end
    end
    Channelnames.Digital = {};
    if ~isempty(board_dig_in_channels)
        for i = 1:length(board_dig_in_channels)
            Channelnames.Digital{i} = board_dig_in_channels(i).custom_channel_name;
        end
    end
...
cN = length(num_amplifier_channels);  % number of steps/chunks
...
if (data_present)
   Textarea = strcat("File contains ",num2str(record_time)," seconds of data.  Amplifiers were sampled at ",num2str(sample_rate / 1000)," kS/s.");
    pause(0.1);
    disp(Textarea);
    fprintf(1, '\n');
else
   Textarea = strcat("Header file contains no data.  Amplifiers were sampled at ",num2str(sample_rate / 1000)," kS/s.");
    pause(0.1);
    disp(Textarea);
    fprintf(1, '\n');
end
...
if (data_present)
   ...
   Textarea = [Textarea;"Allocating memory for data"];
    pause(0.1);
    ...
    for i=1:num_data_blocks
        ...
        if (fraction_done >= percent_done)
           fraction = fraction_done/cN;
           msg = sprintf('Extracting Data... (%d%% done)', percent_done);
           waitbar(fraction/100, h, msg);

           Textarea = [Textarea;strcat(num2str(percent_done),"% done")];
            pause(0.1);
...
if (data_present)
   ...
   Textarea = [Textarea;"Parsing data"];
    pause(0.1);
    ...
    if (num_gaps == 0)
       Textarea = [Textarea;"No missing timestamps in data."];
        pause(0.1);
    else
       Textarea = [Textarea;strcat("Warning: ",num2str(num_gaps)," gaps in timestamp data found.  Time scale will not be uniform!")];
        pause(0.1);
    end
...
board_adc_data = single(board_adc_data);
board_dig_in_data = single(board_dig_in_data);
t_dig = single(t_dig);
aux_input_data = single(aux_input_data);

NrChannel = size(amplifier_data,1);
