...
for nchannel = 1:length(ChannelRange)
    channel = nchannel*Data.Info.ChannelSpacing;
    channel_indices = SpikePositions == channel;
...
if Plot
    ...
    Waveform_handles = findobj('Type', 'line', 'Tag', 'Waveforms');
    ...
    for nchannel = 1:size(Waveforms,1)
        for nwaves = 1:numel(WaveformRange)
            ...
            if isempty(Waveform_handles) && NrPlots > length(Waveform_handles) 
                ...
                plot(Figure,Time,Plotdata, 'Tag', 'Waveforms','LineWidth',2);
            else
                Plotdata = squeeze(Waveforms(nchannel,nwaves,:));
                if size(Plotdata,2) == 1
                    Plotdata = Plotdata';
                end
                set(Figure,Waveform_handles(NrPlots), 'XData', Time, 'YData', Plotdata, 'Tag', 'Waveforms','LineWidth',2);
            end
        end
    end
