function Spike_Waveforms_Plot_Waveforms(Data,Units,Waves,figs)
...
Time = (0:1/Data.Info.NativeSamplingRate:(size(Data.Spikes.Waveforms,2)-1)/Data.Info.NativeSamplingRate)*1000; %IN MS
%% Color map
...
for nplots = 1:length(Units)
    ...
    if length(MeanWaveformhandles)>length(Units{nplots})
        delete(MeanWaveformhandles(length(Units{nplots})+1:end));
        MeanWaveformhandles = findobj(Figurename, 'Tag', 'MeanWaveforms');
    end
    ...
    for nindividualunits = 1:length(Units{nplots}) % Loop over all units per plot
        ...
        Indicies = Data.Spikes.SpikeCluster == Units{nplots}(nindividualunits);

        TempClusterWaves = Data.Spikes.Waveforms(Indicies,:);
