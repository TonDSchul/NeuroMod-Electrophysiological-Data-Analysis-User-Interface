...
if strcmp(WhattoPlot,"Just Frequency Bands") || strcmp(WhattoPlot,"All")
    ...
    PowerForBandsHandles = findobj(FrequencyBandsFigure, 'Tag', 'PowerForBands');

    if length(PowerForBandsHandles)>5
        delete(PowerForBandsHandles(6:end));
    end
    ...
    for q = 1:length(freqBands)
        ...
        if isempty(PowerForBandsHandles)
            plot(FrequencyBandsFigure,thisPow, (0:nC-1)*ChannelSpacing,'LineWidth',2,'Tag','PowerForBands');
        else
            set(PowerForBandsHandles(q),'XData',thisPow,'YData', (0:nC-1)*ChannelSpacing,'LineWidth',2,'Tag','PowerForBands');
        end
    end
