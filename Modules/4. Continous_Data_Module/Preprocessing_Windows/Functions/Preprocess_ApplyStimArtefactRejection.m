function [Data] = Preprocess_ApplyStimArtefactRejection(Data,Info,Events,PreproInfo)
...
Eventsoutside = [];
numtimepoints = round((abs(PreproInfo.TimeAroundStimArtefact(1))+abs(PreproInfo.TimeAroundStimArtefact(2)))*Info.NativeSamplingRate)+1;
TimeBefore(1) = abs(PreproInfo.TimeAroundStimArtefact(1)*Info.NativeSamplingRate);
TimeAfter(1) = abs(PreproInfo.TimeAroundStimArtefact(2)*Info.NativeSamplingRate);
