function [Data] = Execute_Autorun_Extract_Events_Module_Functions(AutorunConfig,FunctionOrder,Data,LoadDataPath,LoadedData)
...
if strcmp(FunctionOrder,'Extract_Events')
    ...
    TextEventExtraction = [];
    [EventInfo,FileEndingsExist,FilesIndex,DatFilePaths,FilesinPath] = Extract_Events_Module_Determine_Available_EventChannel(Data,Data.Info.Data_Path,TextEventExtraction,TextEventExtraction,AutorunConfig.ExtractEventDataModule.ChannelOfInterest);
    ...
    [Data,EventChannelDropDown,RHDAllChannelData,ExtractedRHDEventsFlag] = Extract_Events_Module_Main_Function(Data,EventInfo,Data.Info.Data_Path,Data.Info.RecordingType,AutorunConfig.ExtractEventDataModule.ChannelOfInterest,AutorunConfig.ExtractEventDataModule.EventSignalThreshold,InputChannelSelection,0,TextEventExtraction);
...
if strcmp(FunctionOrder,'Event_Analysis_ERP') || strcmp(FunctionOrder,'Event_Analysis_CSD') || strcmp(FunctionOrder,'Event_Analysis_TimeFrequencyPower')
    ...
    if strcmp(FunctionOrder,'Event_Analysis_ERP')
        ...
        if strcmp(AutorunConfig.SaveFigures,"on")
            Execute_Autorun_Save_Figure(ERPFigure, AutorunConfig.SaveFiguresFormat, AutorunConfig.DeleteFigureAfterSaving, "Event_Related_Potential", LoadDataPath, "ERP", AutorunConfig.ExtractRawRecording.FileType, [], AutorunConfig.ExtractRawRecording.RecordingsSystem, LoadedData, "ERP")
        end
    end
    ...
    if strcmp(FunctionOrder,'Event_Analysis_CSD')
        ...
        %% Plot Results if turned on
        if strcmp(AutorunConfig.SaveFigures,"on")
            Execute_Autorun_Save_Figure(CSDFigure, AutorunConfig.SaveFiguresFormat, AutorunConfig.DeleteFigureAfterSaving, "Event_Related_Current_Source_Density", LoadDataPath, "CSD", AutorunConfig.ExtractRawRecording.FileType, [], AutorunConfig.ExtractRawRecording.RecordingsSystem, LoadedData, "CSD")
        end
    ...
    if strcmp(FunctionOrder,'Event_Analysis_TimeFrequencyPower')
        for i = 1:length(AutorunConfig.AnalyseEventDataModule.TFPlotType)
            for j = 1:length(AutorunConfig.AnalyseEventDataModule.TFPlotAddons)
                ...
                %% Plot Results if turned on
                if strcmp(AutorunConfig.SaveFigures,"on")
                    Execute_Autorun_Save_Figure(TFFigure, AutorunConfig.SaveFiguresFormat, AutorunConfig.DeleteFigureAfterSaving, "_Time_Frequency_Power", LoadDataPath, AutorunConfig.AnalyseEventDataModule.TFPlotType(i), AutorunConfig.ExtractRawRecording.FileType, AutorunConfig.AnalyseEventDataModule.TFPlotAddons(j), AutorunConfig.ExtractRawRecording.RecordingsSystem, LoadedData, "TF")
                end
            end
        end
    end
    ...
    elseif isfield(Data,'Spikes') && strcmp(Data.Info.SpikeType,"Kilosort")
        ...
        if Error == 0 
            ...
            for i = 1:length(AutorunConfig.AnalyseEventSpikesModule.Plottype)
                ...
                %% Plot Results if turned on
                if strcmp(AutorunConfig.SaveFigures,"on")
                    Execute_Autorun_Save_Figure(SpikeAnalysisFigure, AutorunConfig.SaveFiguresFormat, AutorunConfig.DeleteFigureAfterSaving, "Event_Kilosort_", LoadDataPath, AutorunConfig.AnalyseEventSpikesModule.Plottype(i), AutorunConfig.ExtractRawRecording.FileType, [], AutorunConfig.ExtractRawRecording.RecordingsSystem, LoadedData, "KilosortEventSpikes")
                end
    ...
    elseif isfield(Data,'Spikes') && strcmp(Data.Info.SpikeType,"Internal")
        ...
        if Error == 0
            ...
            for i = 1:length(AutorunConfig.AnalyseEventSpikesModule.Plottype)
                ...
                %% Plot if turned on
                if strcmp(AutorunConfig.SaveFigures,"on")
                    Execute_Autorun_Save_Figure(SpikeAnalysisFigure, AutorunConfig.SaveFiguresFormat, AutorunConfig.DeleteFigureAfterSaving, "Event_Internal", LoadDataPath, AutorunConfig.AnalyseEventSpikesModule.Plottype(i), AutorunConfig.ExtractRawRecording.FileType, [], AutorunConfig.ExtractRawRecording.RecordingsSystem, LoadedData, "InternalEventSpikes")
                end
