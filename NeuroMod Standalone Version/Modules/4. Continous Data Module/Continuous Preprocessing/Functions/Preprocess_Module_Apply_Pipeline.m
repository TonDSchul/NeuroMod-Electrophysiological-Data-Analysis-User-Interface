...
%% When Plot Example Later: Just do it for one channel and the whole signal!
% only one second for plot selected in the actual plot function at the
% bottom
if PlotExample == 1 || Inspectfilter == 1
    ExamplePlotchannel = round(size(Data.Raw,1)/2);
    if ~strcmp(PreprocessingSteps(1),"GrandAverage") % for grand average all channel are needed
        Data.Raw = Data.Raw(ExamplePlotchannel,:);
        if isfield(Data,'Preprocessed')
            Data.Preprocessed = Data.Preprocessed(ExamplePlotchannel,:);
        end
    end
end
...
for PPSteps = 1:length(PreprocessingSteps) % Loop thorugh preprocessing steps
    ...
    elseif strcmp(PreprocessingSteps(PPSteps),"Stimulation Artefact Rejection")
        if ~isempty(find(PreprocessingSteps == "Downsampling"))
            ...
            TempSR = Data.Info.NativeSamplingRate;
            Downsampled = 0;
        end

        if PPSteps == 1 % If first step in pipeline: apply to raw data
            Data.Preprocessed = Preprocess_ApplyStimArtefactRejection(Data.Raw,Data.Info,Data.Events,PreProInfo,TempSR,Downsampled);
        else % If not first step in pipeline: apply to already preprocessed data
            Data.Preprocessed = Preprocess_ApplyStimArtefactRejection(Data.Preprocessed,Data.Info,Data.Events,PreProInfo,TempSR,Downsampled);
        end
...
if PlotExample == 1 && Inspectfilter == 0
    ...
    if strcmp(PreprocessingSteps,"Downsampling")
        ...
        plot(Data.Time(TimeToPlot == 1),Data.Raw(1,TimeToPlot == 1),'LineWidth',1.5,'Color',[0, 0.4470, 0.7410]);
        plot(Data.TimeDownsampled(TimeDSToPlot == 1),Data.Preprocessed(1,TimeDSToPlot == 1),'LineWidth',1,'Color',[0.8500, 0.3250, 0.0980]);
    else
        plot(Data.Time(1:Data.Info.NativeSamplingRate),Data.Raw(1,1:Data.Info.NativeSamplingRate),'LineWidth',1,'Color',[0, 0.4470, 0.7410]);
        plot(Data.Time(1:Data.Info.NativeSamplingRate),Data.Preprocessed(1,1:Data.Info.NativeSamplingRate),'LineWidth',1.,'Color',[0.8500, 0.3250, 0.0980]);
    end
    
    legend("Original Signal","Filtered Signal")
    xlabel('Time [s]');
    ylabel('Signal [mV]');
    titlestring = strcat("Channel ",num2str(ExamplePlotchannel)," Original vs. filtered Signal");
    title(titlestring);
